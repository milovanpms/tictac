...
extern uint16_t pas_interupt;
volatile bool dans_pic = false;

static uint32_t dernier_pas_timestamp = 0;
...
void BSP_TIMER_run_us(timer_id_t timer_id, uint32_t us, bool enable_irq)
{
	...
	if(enable_irq) {
	    ...
	    // AVANT : Priorité 4 (trop basse)
	    // APRÈS : Priorité 6 (plus basse que SPI/I2C)
	    HAL_NVIC_SetPriority(nvic_irq_array[timer_id], 6, 1);
...
/**
 * @brief Gestionnaire d'interruption du Timer 4 pour la détection de pas basée sur l'accéléromètre MPU6050
 *
 * Cette fonction est appelée périodiquement par l'interruption du Timer 4. Elle lit les données
 * de l'accéléromètre MPU6050, calcule la magnitude du vecteur d'accélération et détecte les pas
 * en utilisant un algorithme de détection de pics avec temps mort pour éviter les faux positifs.
 *
 * L'algorithme fonctionne en deux phases :
 * - Détection d'un pic : quand la magnitude dépasse le seuil et qu'on n'est pas déjà dans un pic
 * - Sortie du pic : quand la magnitude repasse sous le seuil
 *
 * Un temps mort est appliqué entre chaque détection de pas pour éviter de compter plusieurs fois
 * le même mouvement.
 *
 * @pre Le MPU6050 doit être initialisé et configuré
 * @pre Les variables globales suivantes doivent être définies :
 *      - SEUIL : seuil de détection des pas
 *      - TEMPS_MORT : temps minimum entre deux détections de pas (en ms)
 *      - pas_interupt : compteur de pas (variable globale)
 *      - dans_pic : flag indiquant si on est actuellement dans un pic
 *      - dernier_pas_timestamp : timestamp du dernier pas détecté
 *      - MPU6050_Data : structure contenant les données de l'accéléromètre
 *
 * @post La variable globale pas_interupt est incrémentée si un pas est détecté
 * @post Les variables d'état dans_pic et dernier_pas_timestamp sont mises à jour
 *
 * @return void
 */
__weak void TIMER4_user_handler_it(void)
{
    // Lecture de toutes les données de l'accéléromètre MPU6050
    MPU6050_ReadAll(&MPU6050_Data);

    // Calcul de la magnitude du vecteur d'accélération pour une détection plus fiable
    // La magnitude évite les problèmes d'orientation du capteur
    int32_t magnitude = abs(MPU6050_Data.Accelerometer_X) + abs(MPU6050_Data.Accelerometer_Y) + abs(MPU6050_Data.Accelerometer_Z);

    // Récupération du timestamp actuel pour la gestion du temps mort
    uint32_t maintenant = HAL_GetTick();

    // Détection d'un pic d'accélération (début de pas potentiel)
    if (magnitude > SEUIL)
    {
        // Vérification qu'on n'est pas déjà dans un pic et que le temps mort est écoulé
        if (!dans_pic && (maintenant - dernier_pas_timestamp > TEMPS_MORT))
        {
            pas_interupt++; // Incrémentation du compteur de pas
            dans_pic = true; // Marquage qu'on est maintenant dans un pic
            dernier_pas_timestamp = maintenant; // Sauvegarde du timestamp pour le temps mort
        }
    }
    else
    {
        // La magnitude est retombée sous le seuil, on sort du pic
        dans_pic = false;
    }
}

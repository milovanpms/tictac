...
running_t DEMO_RTC_process_main(bool ask_for_finish)
{
	...
	switch(state)
	{
		...
		case RUN:{
			...
			{
				char buffer[64];
				snprintf(buffer, sizeof(buffer), "%s %02d/%02d/%02d - %02d:%02d:%02d\n",
				         weekday_str[date.WeekDay], date.Date, date.Month, date.Year,
				         time.Hours, time.Minutes, time.Seconds);

				BSP_UART_puts(UART2_ID, buffer, strlen(buffer));
			...
			if(BSP_UART_data_ready(UART2_ID))
			{
				...
				{
					case 'r':
						BSP_UART_puts(UART1_ID, "reset time & date\n", 0);
						...
						break;
					case 'h':
						BSP_UART_puts(UART2_ID, "This demo will print the time every second.\n", 0);
						BSP_UART_puts(UART2_ID, "Commands :\n", 0);
						BSP_UART_puts(UART2_ID, "h : help\n", 0);
						BSP_UART_puts(UART2_ID, "r : reset time & date to default\n", 0);
						break;
				}
			}
			if(flag_alarm)
			{
				BSP_UART_puts(UART2_ID, "alarm occured\n", 0);
				flag_alarm = false;
			}
			if(ask_for_finish)
			{
